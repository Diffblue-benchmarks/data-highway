...
public class KafkaAdminClient {
  ...
  public List<PatchOperation> updateMessageStatus(KafkaRoad road) {
    ...
    if (topicName != null && topicExists(topicName)) {
      ...
      log.info(
          "Will skip updating MessageStatus because model Topic name is null or Topic is not yet created in Kafka");
    }
  ...
  private MessageStatus getMessageStatus(String topicName) {
    ...
    List<TopicPartition> topicPartitions = Flux.fromIterable(map.keySet())
        .map(m -> new TopicPartition(topicName, (Integer) m))
        .collectList()
        .block();
